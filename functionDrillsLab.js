...
function nameCheck(name) {
  const greet1 = `What is up ${name}?` 
  const greet2 = `Hey ${name}!`
  const greet3 = `Cool name, ${name}!`
  if (name === "Steven") {
    return greet1
  } else if ( name === "Bryan") {
    return greet2
...
let nameGreeting = nameCheck("Clay");
...
colorRating = faveColorFinder("purple");
...
function logString(arg) {
  let upper = arg.toUpperCase()
  return upper
}

console.log(logString("k"));
...
console.log(emailCheck('         clay2000lbsgmail.com'))
...
function frogForGold(gold) {
  return Math.floor(numFrogs = gold / 3);
}

let totalFrogs = frogForGold(20);
...
/*solved in the previous problem
